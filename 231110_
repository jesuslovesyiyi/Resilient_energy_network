...
file_dir = 'D:/ESMI_Dataset/ESMI_Data/IND_EAR5_Land/IND_EAR5_Land_NetCDF/'
output_dir = 'D:/ESMI_Dataset/ESMI_Data/IND_EAR5_Land/output/'
station_data_file = "D:/ESMI_Dataset/ESMI_Data/IND_EAR5_Land/ESMI_Location_IND.csv"
checkpoint_file = "D:/ESMI_Dataset/ESMI_Data/IDN_EAST_EAR5_Land/checkpoint.pkl"

# Check if a checkpoint file exists
if os.path.exists(checkpoint_file):
    checkpoint_data = pd.read_pickle(checkpoint_file)
    last_processed_station = checkpoint_data['station_id'].iloc[-1]
    resume_index = checkpoint_data.index.get_loc(checkpoint_data['station_id'].last_valid_index()) + 1
    print(f"Resuming from station {last_processed_station}")
else:
    resume_index = 0
...
for index, station_row in station_info_df.iterrows():
    ...
    if all_data:
        ...
        # Save checkpoint
        checkpoint_data = pd.concat([checkpoint_data, station_data_df], ignore_index=True)
        checkpoint_data.to_pickle(checkpoint_file)
