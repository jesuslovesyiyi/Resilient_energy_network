...
def retry_request(url, session, total=4, status_forcelist=[429, 500, 502, 503, 504], wait_time=0, **kwargs):
    ...
    for _ in range(total):
        try:
            ...
            if wait_time:
                response.html.render(wait=wait_time)
            return response
    ...
    for id in stationIDs:
        ...
        with tqdm(total=(range_end - range_start).days) as pbar:
            while query_end <= range_end and query_start < query_end:
                ...
                res = retry_request(url=url, session=session, headers=HEADERS, wait_time=0.5)
